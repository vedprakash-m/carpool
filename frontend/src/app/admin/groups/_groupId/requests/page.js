...
exports.default = JoinRequestsPage;
...
const mockRequests = [
    {
        ...
        message: "We're excited to join! I can drive Tuesday and Thursday mornings regularly.",
        impact: {
...
function JoinRequestsPage() {
    ...
    const handleDecision = (familyId, decision) => {
    ...
    return (<div className="p-8">
      ...
      {requests.length === 0 ? (<p className="text-gray-500">No pending join requests.</p>) : (<div className="space-y-6">
          {requests.map((req) => (<div key={req.familyId} className="bg-white p-6 rounded-lg shadow-md border">
              ...
              <div className="mt-4 pt-4 border-t flex justify-end space-x-4">
                <button onClick={() => handleDecision(req.familyId, "decline")} className="btn-secondary">
                  <outline_1.XMarkIcon className="h-5 w-5 mr-2"/>
                  Decline
                </button>
                <button onClick={() => handleDecision(req.familyId, "approve")} className="btn-primary">
                  <outline_1.CheckIcon className="h-5 w-5 mr-2"/>
                  Approve Family
                </button>
              </div>
            </div>))}
