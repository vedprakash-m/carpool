...
class AccessibilityService {
    ...
    announceLive(message, priority = "polite") {
        if (!this.liveRegion)
            return;
    ...
    addAccessibilityAttributes(element, options) {
        if (options.label) {
...
function useAccessibility() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        return () => {
            mediaQueries.forEach((mq) => mq.removeEventListener("change", handlePrefChange));
        };
    }, []);
    const announceLive = (message, priority) => {
    ...
    const validateContrast = (foreground, background) => {
        return (serviceRef.current?.validateColorContrast(foreground, background) || {
            ratio: 0,
            wcagAA: false,
            wcagAAA: false,
        });
...
exports.default = AccessibilityService;
