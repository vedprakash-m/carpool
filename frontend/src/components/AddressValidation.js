...
exports.default = AddressValidation;
const react_1 = require("react");
const auth_store_1 = require("@/store/auth.store");
const outline_1 = require("@heroicons/react/24/outline");
function AddressValidation({ onValidationComplete, required = false, }) {
    ...
    const validateAddress = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setMessage({
                    type: "success",
                    text: `Address verified! Distance to ${result.serviceArea.school}: ${result.distanceToSchool.toFixed(1)} miles`,
                });
                ...
                const { errorCode, message: errorMessage, suggestions: errorSuggestions, } = data.error;
    ...
    if (status?.verified) {
        ...
        <div className="flex items-start space-x-3">
          <outline_1.CheckCircleIcon className="h-6 w-6 text-green-600 flex-shrink-0 mt-0.5"/>
          <div>
    ...
    return (<div className="bg-white border border-gray-200 rounded-lg p-6">
      <div className="flex items-center space-x-3 mb-4">
        <outline_1.MapPinIcon className="h-6 w-6 text-blue-600"/>
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="relative">
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="123 Main St, Redmond, WA 98052" className="w-full px-3 py-2 pr-10 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
            <outline_1.MagnifyingGlassIcon className="absolute right-3 top-2.5 h-5 w-5 text-gray-400"/>
          </div>
        ...
        {suggestions.length > 0 && (<div className="border border-gray-200 rounded-md">
            <div className="p-3 bg-gray-50 border-b border-gray-200">
              <p className="text-sm font-medium text-gray-700">Did you mean:</p>
            </div>
            <div className="divide-y divide-gray-200">
              {suggestions.map((suggestion, index) => (<button key={index} onClick={() => selectSuggestion(suggestion)} className="w-full px-3 py-2 text-left hover:bg-gray-50 text-sm text-gray-700">
                  {suggestion}
                </button>))}
            </div>
          </div>)}

        <button onClick={validateAddress} disabled={validating || !address.trim()} className="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">
          {validating ? (<>
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
              <span>Validating...</span>
            </>) : (<>
              <outline_1.MapPinIcon className="h-4 w-4"/>
