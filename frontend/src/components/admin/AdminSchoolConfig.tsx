...
import { School, GradeConfig, SUPPORTED_GRADES, DEFAULT_SCHOOLS } from '@/config/schools';
...
export default function AdminSchoolConfig({ onSchoolsChange, onGradesChange }: AdminSchoolConfigProps) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      <div className="bg-white shadow-lg rounded-lg p-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">School & Grade Configuration</h1>
        <p className="text-gray-600 mb-8">
          Configure supported schools and grades for the VCarpool platform. These will appear as dropdown options during parent registration.
        </p>
          ...
          <div className="grid gap-4">
            {schools.map((school) => (
              <SchoolCard
                ...
                onToggleActive={(active) => updateSchool(school.id, { isActive: active })}
              />
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-800">Supported Grades</h2>
            <button
...
const SchoolCard: React.FC<SchoolCardProps> = ({ school, onEdit, onDelete, onToggleActive }) => (
  <div className={`border rounded-lg p-4 ${school.isActive ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'}`}>
    <div className="flex items-start justify-between">
      <div className="flex-1">
        <div className="flex items-center space-x-2 mb-2">
          <h3 className="font-semibold text-gray-900">{school.name}</h3>
          <span className={`px-2 py-1 text-xs rounded-full ${
            school.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
          }`}>
        ...
        <p className="text-sm text-gray-600">Service Radius: {school.serviceRadius} miles</p>
      </div>
  ...
  <div className="border border-gray-200 rounded-lg p-4 hover:border-gray-300 transition-colors">
    <div className="flex items-start justify-between">
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-600 mb-2">Numeric: {grade.numericValue}</p>
        <div className="flex flex-wrap gap-1">
          {grade.schoolTypes.map((type) => (
            <span key={type} className="px-2 py-1 text-xs bg-blue-100 text-blue-800 rounded-full capitalize">
              {type}
            </span>
          ))}
        </div>
...
const SchoolFormModal: React.FC<SchoolFormModalProps> = ({ school, onSave, onCancel }) => {
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Supported Grades *</label>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                {SUPPORTED_GRADES.map((grade) => (
                  <label key={grade.id} className="flex items-center space-x-2 text-sm">
                    <input
                      type="checkbox"
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <label className="text-sm font-medium text-gray-700">Active (available for registration)</label>
            </div>
...
const GradeFormModal: React.FC<GradeFormModalProps> = ({ grade, onSave, onCancel }) => {
  const [formData, setFormData] = useState({
    label: grade?.label || '',
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({...formData, numericValue: parseInt(e.target.value)})}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">School Types *</label>
