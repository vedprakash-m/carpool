...
exports.default = PreferenceTutorial;
const react_1 = __importStar(require("react"));
const outline_1 = require("@heroicons/react/24/outline");
function PreferenceTutorial({ onNext, onPrevious, onComplete, }) {
    const [currentDemo, setCurrentDemo] = (0, react_1.useState)(0);
    const demoSteps = [
        {
            title: "Weekly Schedule Overview",
            description: "You'll see a weekly calendar with morning and afternoon time slots",
            content: (<div className="bg-gray-50 rounded-lg p-4">
        ...
        },
        {
            ...
            description: "Click on time slots to set: Can't Drive, Prefer Not, Available, or Prefer",
        ...
        },
        {
            ...
            description: "Follow these tips to help create fair and efficient schedules",
            content: (<div className="space-y-3">
          <div className="flex items-start space-x-3">
            <outline_1.CheckCircleIcon className="w-5 h-5 text-green-600 mt-0.5 flex-shrink-0"/>
            <div>
    ...
    return (<div className="space-y-6">
      <div className="text-center mb-6">
        <outline_1.CalendarIcon className="w-12 h-12 text-indigo-600 mx-auto mb-3"/>
      ...
      <div className="flex justify-center space-x-2 mb-6">
        {demoSteps.map((_, index) => (<div key={index} className={`w-3 h-3 rounded-full ${index === currentDemo
                ? "bg-indigo-600"
                : index < currentDemo
                    ? "bg-green-600"
                    : "bg-gray-300"}`}/>))}
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-start">
          <outline_1.ExclamationTriangleIcon className="w-5 h-5 text-yellow-600 mr-3 mt-0.5"/>
          <div>
      ...
      <div className="flex justify-between items-center">
        <button onClick={handlePrevious} disabled={currentDemo === 0} className={`px-4 py-2 rounded-lg transition-colors ${currentDemo === 0
        ...
        <button onClick={handleNext} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">
          {currentDemo === demoSteps.length - 1 ? "Got It!" : "Next"}
        </button>
      </div>

      {/* Ready to Try */}
      {currentDemo === demoSteps.length - 1 && (<div className="text-center bg-indigo-50 rounded-lg p-4 border border-indigo-200">
          <outline_1.ClockIcon className="w-8 h-8 text-indigo-600 mx-auto mb-2"/>
