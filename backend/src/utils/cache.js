...
function Cacheable(keyGenerator, ttl) {
    return function (target, propertyName, descriptor) {
        ...
        descriptor.value = async function (...args) {
            ...
            const cache = exports.globalCache; // Type assertion for singleton
...
class CacheWarmer {
    ...
    static async warmupUserData(userId, userService) {
        try {
            ...
            logger_1.logger.warn("Failed to warm up user data", {
                userId,
    ...
    static async warmupPopularSearches(carpoolService) {
        ...
        for (const search of popularSearches) {
            try {
                ...
                logger_1.logger.warn("Failed to warm up search data", {
                    search,
                    error: error instanceof Error ? error.message : "Unknown error",
                });
            }
