...
class TripService {
  ...
  static async getTripById(tripId) {
    const { containers } = await Promise.resolve().then(() =>
      __importStar(require("../config/database"))
    );
  ...
  async getTripById(tripId) {
    try {
      ...
      throw error_handler_1.Errors.InternalServerError(
        `Error fetching trip: ${error.message}`
      );
    }
  }
  /**
  ...
  async addPassenger(tripId, passengerId, pickupLocation) {
    try {
      ...
      if (trip.passengers.includes(passengerId)) {
        throw error_handler_1.Errors.Conflict(
          "User is already a passenger on this trip"
        );
      }
      if (trip.availableSeats <= 0) {
        throw error_handler_1.Errors.BadRequest(
  ...
  async removePassenger(tripId, passengerId) {
    try {
      ...
      if (!trip.passengers.includes(passengerId)) {
        throw error_handler_1.Errors.BadRequest(
          "User is not a passenger on this trip"
        );
      }
      const updatedTrip = {
        ...trip,
  ...
  async getTripStats(userId) {
    try {
      ...
      const tripsAsPassenger = trips.filter(
        (trip) => trip.passengers.includes(userId) && trip.driverId !== userId
      );
      // Calculate miles and time savings instead of cost
      const totalMiles = trips.reduce((total, trip) => {
      ...
      console.error(
        `Error fetching trip statistics for user ${userId}:`,
        error
      );
      throw error_handler_1.Errors.InternalServerError(
   ...
   */
  async joinTrip(tripId, userId, pickupLocation) {
