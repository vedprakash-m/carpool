...
describe("Universal School Support System", () => {
  describe("School Configuration", () => {
    it("should support multiple schools with different service areas", () => {
      ...
      const elementarySchool = DEFAULT_SCHOOLS.find(
        (s) => s.type === "elementary"
      );
  ...
  describe("Smart School Detection", () => {
    ...
    it("should detect nearby schools based on coordinates", () => {
      const detectNearbySchools = (coordinates: {
        latitude: number;
        longitude: number;
      }) => {
      ...
      if (nearbySchools.length > 1) {
        expect(nearbySchools[0].distance).toBeLessThanOrEqual(
          nearbySchools[1].distance
        );
      }
    });
  ...
  describe("Registration Form Validation", () => {
    it("should validate smart registration data", () => {
      ...
      expect(smartRegistrationData.detectedSchool?.confidence).toBeGreaterThan(
        0.8
      );
  ...
  describe("Legacy Compatibility", () => {
    it("should maintain Tesla STEM support for existing code", () => {
      const { TESLA_STEM_HIGH_SCHOOL } = require("@shared/types");
