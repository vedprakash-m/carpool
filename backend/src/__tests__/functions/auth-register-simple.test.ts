...
describe("Auth Register - Family-Oriented Core Validation", () => {
  describe("Family Email Validation", () => {
    ...
    it("should extract school domains for family community verification", () => {
      ...
      const isSchoolCommunity = (domain: string): boolean =>
        /\.(edu|k12\.us|org)$/.test(domain);
  ...
  describe("Family Role Assignment", () => {
    it("should validate family-oriented role assignments", () => {
      ...
      expect(validateFamilyRole("admin").permissions).toContain(
        "emergency_coordinator"
      );
      expect(validateFamilyRole("group_admin").permissions).toContain(
        "group_management"
      );
      expect(validateFamilyRole("parent").permissions).toContain(
        "family_management"
      );
    });
  });
  ...
  describe("Family Registration Data", () => {
    it("should validate required family registration fields", () => {
      const validateFamilyRegistration = (
        userData: FamilyRegistrationData
      ): ValidationResult => {
        ...
        const hasOptionalFamily = familyOptionalFields.some(
          (field) => userData[field]
        );
        ...
        return {
          ...
          familyRegistrationType: hasOptionalFamily
            ? "full_family"
            : "basic_parent",
        };
      };
  ...
  describe("Emergency Contact Validation", () => {
    it("should validate emergency contact requirements for family safety", () => {
      const validateEmergencyContacts = (
        ...
        const hasPickupAuth = contacts.some(
          (contact) => contact.canPickup === true
        );
